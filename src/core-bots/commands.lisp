...
(defproto =command-bot= (=listener-bot=)
...
(defreply update-detection-regex ((bot =command-bot=))
  (setf (detection-regex bot)
        (create-scanner (build-string "^(?:~A[:,] |~A)" (nickname bot) (command-prefix bot))
                        :case-insensitive-mode T)))

(defreply (setf command-prefix) :after (new-value (bot =command-bot=))
  "Keeps the detection-regex up-to-date."
  (declare (ignore new-value))
  (update-detection-regex bot))

(defreply (setf nickname) :after (new-value (bot =command-bot=))
  "Keeps the detection-regex up-to-date."
  (declare (ignore new-value))
  (update-detection-regex bot))
...
(defreply add-command ((bot =command-bot=) name command)
  (setf (gethash name (commands bot)) command))

(defreply remove-command ((bot =command-bot=) name)
  (remhash name (commands bot)))

(defreply find-command ((bot =command-bot=) name)
  (with-properties (commands) bot
    (gethash (string-downcase name) commands)))

(defreply command-function ((bot =command-bot=) name)
  (or (let ((cmd (find-command bot name)))
	(when cmd (cmd-function cmd)))
      (error 'unknown-command :command-name name)))

(defreply erase-all-commands ((bot =command-bot=))
  (clrhash (commands bot)))

(defreply list-all-commands ((bot =command-bot=))
  (with-properties (commands) bot
...
(defmacro defcommand (name (&optional (regex "") &rest vars) &body body)
  (let ((documentation nil)
	...
	(setf real-body body))
    `(add-command =command-bot= (string-downcase (symbol-name ',name))
                  (defobject (=command=)
                      ((cmd-function
                        (lambda ()
                          ,@(if vars
...
(defreply get-message-index ((bot =sykobot=) message)
